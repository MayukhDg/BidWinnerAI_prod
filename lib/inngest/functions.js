...
export const generateRFPAnswers = inngest.createFunction(
  {
    id: 'generate-rfp-answers',
    name: 'Generate RFP Answers',
    concurrency: 5,
  },
  { event: 'rfp/generate.answers' },
  async ({ event, step }) => {
    const { rfpId } = event.data;

    const questions = await step.run('get-questions', async () => {
      const questionsCollection = await getCollection('rfpQuestions');
      const qs = await questionsCollection.find({ 
        rfpId: new ObjectId(rfpId),
        status: 'pending'
      }).toArray();
      return qs.map(q => ({ _id: q._id.toString(), question: q.question, userId: q.userId.toString() }));
    });

    for (const q of questions) {
      await step.run(`answer-question-${q._id}`, async () => {
        // 1. Search context
        const contextChunks = await searchSimilarChunks(q.question, q.userId);
        ...
        await questionsCollection.updateOne(
          ...
          { $set: { answer, status: 'drafted', updatedAt: new Date() } }
        );
      });
    }
...
export const functions = [processDocument, handleProcessingFailure, generateRFPAnswers];
